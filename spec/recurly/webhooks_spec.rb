  ...
  describe "verify_signature" do
    ...
    context "with current notification" do
      it "raises no error" do
        expect { Recurly::Webhooks.verify_signature(header, secret: secret, body: body) }.
          not_to raise_error
      end

      it "raises error if secrets mismatched" do
        expect { Recurly::Webhooks.verify_signature(header, secret: "key", body: body) }.
          to raise_error(Recurly::Errors::SignatureVerificationError, /matching signature/)
      end

      it "raises error if payload body is altered" do
        expect { Recurly::Webhooks.verify_signature(header, secret: secret, body: "{}") }.
          to raise_error(Recurly::Errors::SignatureVerificationError, /matching signature/)
      end
      ...
      it "raises error if header timestamp is altered" do
        ...
        expect { Recurly::Webhooks.verify_signature("#{altered_ts},#{signature}", secret: secret, body: body) }.
          to raise_error(Recurly::Errors::SignatureVerificationError, /matching signature/)
      end
    end
    ...
    context "with notification older than five minutes" do
      ...
      it "raises no error when tolerance is large enough" do
        expect { Recurly::Webhooks.verify_signature(header, secret: secret, body: body, tolerance: Float::INFINITY) }.
          not_to raise_error
      end
    end
