...
RSpec.describe Recurly::HTTP::DefaultHttpAdapter do
  ...
  describe "transport exceptions map to TransportError#kind" do
    def expect_kind(raised, kind)
      ...
      expect { adapter.call("GET", url, {}, nil) }
        .to raise_error(Recurly::Errors::TransportError) { |e| expect(e.kind).to eq(kind) }
    end
    ...
    it "carries the underlying exception as #cause" do
      ...
      expect { adapter.call("GET", url, {}, nil) }
        .to raise_error(Recurly::Errors::TransportError) { |e| expect(e.cause).to be_a(Errno::ECONNREFUSED) }
    end
  end
end
